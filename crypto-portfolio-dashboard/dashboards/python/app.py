...
def render_overview():
    ...
    if isinstance(ov, dict) and ov.get("top_holdings"):
        ...
        # --- NEW: attach latest prices per symbol ---
        # Try to get prices from the ov payload first (expects a dict: {symbol: price})
        price_map = pd.Series(ov.get("last_prices", {}), dtype=float)

        # If ov doesn't have them, fall back to your loader (implement this in src/io/loaders.py)
        if price_map.empty:
            try:
                # should return a pd.Series indexed by symbol -> last price
                from src.io.loaders import load_last_prices
                symbols_needed = th["symbol"].unique().tolist()
                price_map = load_last_prices(symbols=symbols_needed).astype(float)
            except Exception:
                price_map = pd.Series(index=th["symbol"].unique(), dtype=float)

        # Add price to the plotting dataframe (OTHERS will become NaN, which is fine)
        plot_df["price"] = plot_df["symbol"].map(price_map)

        # Simple currency formatting helper (keeps small-coin precision reasonable)
        def _fmt_price(x):
            if pd.isna(x):
                return None
        ...
        with col2:
            ...
            table["price"] = table["price"].apply(_fmt_price)  # format for display

            # Order: symbol | weight % | price
            st.dataframe(
                table[["symbol", "weight %", "price"]],
